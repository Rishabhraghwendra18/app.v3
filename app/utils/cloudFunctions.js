...
Moralis.Cloud.beforeSave("UserInfo", async (request) => {
  ...
  if (existingUser) {
    ...
    if (
      request.object.get("organizationId") !==
      existingUser.get("organizationId")
    ) {
  ...
  if (!request.master) {
    if (
      request.object.get("createdBounties") !==
        existingUser.get("createdBounties") ||
      request.object.get("freelancedBounties") !==
        existingUser.get("freelancedBounties") ||
      request.object.get("completedBounties") !==
        existingUser.get("completedBounties") ||
...
async function validateStatusChange(oldStatus, newStatus) {
  ...
  else if (oldStatus === 202)
    return newStatus === 203 || newStatus === 403 || newStatus === 204;
  else if (oldStatus === 204)
    return newStatus === 203 || newStatus === 403 || newStatus === 202;
...
Moralis.Cloud.define("getUserFeedback", async (request) => {
  var query = new Moralis.Query("Bounty");
  const pipeline = [
    { match: { freelancer: request.params.username } },
    {
      lookup: {
        from: "UserInfo",
        localField: "clientUsername",
